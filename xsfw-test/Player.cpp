...
bool doCollision(Player & player, Ball & ball)
{
	...
	if (hit.penetrationDepth > 0)
	{
		dynamic_resolution(player.transform.position,player.rigidbody.velocity,player.rigidbody.mass,ball.transform.position,ball.rigidbody.velocity,ball.rigidbody.mass,hit);
		return true;
	}
	return false;
}
